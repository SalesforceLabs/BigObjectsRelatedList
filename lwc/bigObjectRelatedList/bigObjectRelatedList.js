...
export default class BigObjectRelatedList extends LightningElement {
    ...
    loadRecords(fields) {
        const trimmedObjectApiName = this.removeWhitespaces(this.apiName);
        const trimmedRetrievedFields = this.removeWhitespaces(this.retrievedFields);
        const trimmedLookupFieldApiName = this.removeWhitespaces(this.lookupFieldApiName);
        return getRecords({ bigObjectApiName: trimmedObjectApiName, fieldsApiNames: trimmedRetrievedFields, lookupFieldApiName: trimmedLookupFieldApiName, parentRecordId: this.recordId, numberOfRecords: this.maxRecords })
            .then(data => {
